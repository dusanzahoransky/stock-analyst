...
import com.github.dusanzahoransky.stockanalyst.util.FormattingUtils
...
class StockAnalysisService {
    ...
    /**
     * Enhance the stock data with Growth and other ratios that were not provided by Yahoo finance API.
     */
    fun calcStockStats(stock: Stock)/*: StockDto*/ {

            val dto = StockDto(stock.symbol, stock.exchange)
            calcFinancialStats(stock, dto)
//            calcChartData(stock, dto)
    ...
    private fun dataAtInterval(
        ...
        return StockChartData(FormattingUtils.localDateToEpochSec(currentInterval), priceAtInterval)
    }

    private fun calcChart(chart: ChartResponse, stock: Stock, samplingInterval: Period) {
...
//        stock.chartData = chartData
    ...
    private fun calcFinancialStats(stock: Stock, dto: StockDto) {
...
//    }
//
//    private fun calcChartData(stock: Stock, dto: StockDto) {
//
...
//        stock.chartData = chartData
    ...
    fun chartDataFirstBefore(date: LocalDate, chartData: List<StockChartData>): StockChartData? {
        return chartData.firstOrNull {
            ...
            chartDate.isBefore(date) && ChronoUnit.DAYS.between(chartDate, date) <= 7
        }
    }

    fun calcStocksAverages(stocks: List<StockDto>): StockDto {

        val counter = StocksAveragesCounter(StockDto("Avg.", Exchange.NASDAQ))
        val averages = counter.averages

        val stockNumericFields = StockDto::class.memberProperties
    ...
    fun combineWithRatios(stocks: List<StockDto>, ratios: List<StockRatiosTimeline>): List<StockWithRatios> {
